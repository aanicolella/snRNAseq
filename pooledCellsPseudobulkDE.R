library("DESeq2")
filepath <- "parentDir/totalExpression_PROJECT_geno1WT_DATE.csv"

rsem.in <- as.matrix(read.csv(filepath, row.names=1))
head(rsem.in)
colData = data.frame(row.names=colnames(rsem.in),model=c(rep(c("GENE_geno1"), 4), rep(c("GENE_WT"), 4)),sample=colnames(rsem.in))
...
pdf(file="GENE_PROJECT_geno1WT_volcano_DATE.pdf")              
high <- row.names(res)[1:10]
high
                         
par(mfrow=c(1,1))
with(res, plot(log2FoldChange, -log10(pvalue), pch=20, main="GENE_PROJECT_Pseudobulk_geno1WT"))
...
res["GENE",]
write.csv(res, "GENE_PROJECT_geno1WT-DESeq2_DATE.csv")    
